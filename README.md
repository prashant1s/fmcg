...
- **Color:** near-black `ink` + warm off-white `paper` as the base, with `blue` (#1A66FF, matching the logo) as the single brand accent — used for CTAs, metrics, and category tagging. Full ramps are defined in [`tailwind.config.ts`](./tailwind.config.ts).
