...
export function ProjectCard({ project, onClick, priority = false, className }: ProjectCardProps) {
  ...
  return (
    <Wrapper
      onClick={onClick}
      className={cn(
        "group relative block aspect-[4/5] w-full overflow-hidden rounded-lg bg-ink-950 text-left",
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-4 p-6">
        ...
        <h3 className="text-xl font-semibold leading-snug text-paper sm:text-2xl">
          {project.title}
        </h3>

        <div className="grid max-h-0 grid-cols-2 gap-4 overflow-hidden opacity-0 transition-all duration-500 ease-expo group-hover:max-h-24 group-hover:opacity-100">
          {project.results.slice(0, 2).map((result) => (
            <div key={result.label} className="border-t border-paper/15 pt-3">
              ...
              <p className="text-[0.7rem] text-paper/50">{result.label}</p>
            </div>
          ))}
        </div>
